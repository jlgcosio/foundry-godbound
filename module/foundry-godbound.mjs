...
Hooks.once('init', function () {
  ...
  Actors.registerSheet('foundry-godbound', GodboundActorSheet, {
    makeDefault: true,
    label: 'GODBOUND.SheetLabels.Actor',
  });
  Items.unregisterSheet('core', ItemSheet);
  Items.registerSheet('foundry-godbound', GodboundItemSheet, {
    makeDefault: true,
...
async function createItemMacro(data, slot) {
  ...
  const command = `game.foundrygodbound.rollItemMacro("${data.uuid}");`;
  ...
  if (!macro) {
    macro = await Macro.create({
      ...
      img: item.img,
      command: command,
      flags: { 'foundry-godbound.itemMacro': true },
    });
  }
